...
const acceptText =
  'Excel 表头（首行）：real_name, identity_no, department, account_name, account_type, permission_type, valid_until'
...
async function onPreview() {
  ...
  try {
    const r = await accountBatchApi.importPreview(id, file.value)
    preview.value = r
    sampleColumns.value = r.sample.length ? Object.keys(r.sample[0]) : []
    if (r.invalid_rows > 0) ElMessage.warning(`有 ${r.invalid_rows} 行错误`)
...
async function onCommit() {
  ...
  try {
    await accountBatchApi.importCommit(id, preview.value.import_token, strategy.value)
    ElMessage.success('导入完成')
    router.push(`/account-batches/${id}`)
  } finally {
...
<template>
  <el-card shadow="never">
    <template #header><span>导入账号名单（批次 #{{ id }}）</span></template>
      ...
      <el-form label-width="120px" inline>
        <el-form-item label="导入策略">
          <el-radio-group v-model="strategy">
            <el-radio value="skip">仅新增（重复跳过）</el-radio>
